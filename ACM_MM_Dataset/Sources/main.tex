...
%% Abstract
\begin{abstract}
We present \ADSMM, a multimodal cross-national educational resource dataset spanning {\NumCourses} courses from {\NumUniversities} universities across {\NumCountries} countries (USA, Sweden, UK, Russia), augmented with structured multimedia resource annotations from MIT OpenCourseWare and instructional activity metadata from MISIS.
...
Cross-institutional transfer (Task 3) is not evaluated due to the single-institution annotation constraint; this remains an open task.
Multimodal baselines combining text + image features (e.g., CLIP) are a natural next step enabled by the released data.
