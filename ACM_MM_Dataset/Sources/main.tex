...
\emph{Learner interaction} datasets (KDD Cup 2010~\cite{stamper2010kddcup}, EdNet~\cite{choi2020ednet}, OULAD~\cite{kuzilek2017oulad}) track student behavior at a single institution.
\emph{MOOC catalog} datasets aggregate course metadata: MOOCCubeX~\cite{yu2023mooccubex} provides 230K video subtitles and 358K exercises from XuetangX---the largest multimodal educational dataset---but covers a single Chinese platform without cross-national scope or labor-market integration.
\emph{Skill mapping} datasets like the Course-Skill Atlas~\cite{courseskillatlas2024} (3M syllabi, 3K US institutions) link curricula to O*NET skills at scale, but contain no multimedia annotations.
\emph{Lecture-level} datasets like M3AV~\cite{m3av2024} (367 hours, audio+video+slides) provide dense multimodal annotations but for individual lectures, not cross-institutional curricula.
...
\textit{Output:} Predicted resource richness score (0--6).
...
\paragraph{Open Task: Cross-Institutional Transfer.}
The cross-national structure enables a transfer task: train on MIT's multimedia annotations, predict resource profiles for non-MIT universities. We do not evaluate this task due to single-institution annotation constraints, but the latent space analysis (Section~\ref{subsec:latent}) provides preliminary evidence of cross-institutional structure.
...
We evaluate three baseline approaches on the cross-modal tasks using {\JoinedCourses} courses with both text descriptions and multimodal annotations (80/20 train/test split, seed=42).
...
TF-IDF features (5,000 terms) from course descriptions, with ridge regression for richness prediction and per-label logistic regression for classification.

\paragraph{Sentence-BERT + balanced classifiers.}
MiniLM sentence embeddings~\cite{reimers2019sentencebert} (384d) with ridge regression for richness and class-weight-balanced logistic regression for per-label classification.
...
Per-label results show meaningful prediction for all three non-trivial resource types: video ($F_1{=}{\SBERTVideoF}$), notes ($F_1{=}{\SBERTNotesF}$), and assessment ($F_1{=}{\SBERTAssessF}$).
This demonstrates that course text descriptions encode significant signal about multimedia instructional design.

\subsection{Discussion}

With SBERT embeddings and balanced class weights, per-label classification becomes viable: video $F_1{=}{\SBERTVideoF}$ despite only 15.3\% positive rate demonstrates that course descriptions contain implicit signals about whether lecture recordings exist.
Notes prediction ($F_1{=}{\SBERTNotesF}$) and assessment prediction ($F_1{=}{\SBERTAssessF}$) are even stronger, consistent with these resource types correlating with text-describable course structure.

\paragraph{Visual modality.}
All {\NumImages} MIT OCW courses include a cover image (Figure~\ref{fig:image-grid}), providing a visual modality for future cross-modal research (e.g., predicting course content from cover images using CLIP-style models).
We release all images with the dataset.
...
To test whether the dataset supports a \emph{unified didactic space}---a shared embedding space where courses, occupations, and missions from heterogeneous institutions coexist---we embed all 36{,}192 canonical records with Sentence-BERT (all-MiniLM-L6-v2, 384d) and analyze the resulting geometry.

\paragraph{Institutions overlap, not cluster.}
We measure the separation ratio (mean inter-institution distance / mean intra-institution distance). A ratio ${<}2$ indicates institutions overlap in embedding space rather than forming isolated clusters. The observed ratio is \textbf{0.70}, confirming a unified space. The closest institution pair is MIT--Stanford (distance 0.20); the most distant is MISIS--ASU (0.94), reflecting language and curricular tradition differences.

\paragraph{Cross-national nearest neighbors.}
For 200 randomly sampled MIT courses, we compute the 10 nearest neighbors and record their institution of origin. \textbf{36.4\%} of MIT's nearest neighbors come from other institutions: UC Berkeley (21.5\%), KTH Sweden (5.5\%), Stanford (5.3\%), Cornell (1.8\%). This demonstrates that semantic similarity crosses institutional and national boundaries.

\paragraph{The primary latent axis encodes multimedia modality.}
PCA on the MIT OCW subset reveals that the \textbf{first principal component correlates with multimedia resource richness} ($r{=}0.41$, $p{<}0.001$). A second independent axis (PC3) also correlates ($r{=}{-}0.23$). Courses with video lectures are displaced from non-video courses (centroid shift = 0.17). This means the embedding space---trained only on text---captures multimedia structure as a geometric property.

\paragraph{MISIS: instructional modality in latent space.}
Courses with laboratory work occupy a different latent region than lecture-only courses (centroid shift = 0.09), and MISIS courses form the tightest cluster (intra-distance 0.54 vs.\ MIT's 0.90), consistent with centralized FGOS standards producing more homogeneous curricula than decentralized US departments.

\paragraph{Courses and jobs share latent structure.}
The course--occupation centroid distance is 0.60, while the course--mission distance is only 0.40---universities successfully encode their missions into curriculum content. KTH is closest to the labor market centroid (0.64), consistent with its applied engineering tradition.
...
The dataset spans {\NumCountries} countries but remains US-heavy (73\% of courses).
O*NET reflects US labor market structure.
MISIS course descriptions are bilingual (Russian/English) for international programs; monolingual Russian courses include English-language competency codes but Russian-only descriptions.
...
Third, courses from 8 universities form a \emph{unified didactic space} (separation ratio 0.70) where 36\% of nearest neighbors cross institutional boundaries, suggesting that curricular content transcends institutional and national context.
