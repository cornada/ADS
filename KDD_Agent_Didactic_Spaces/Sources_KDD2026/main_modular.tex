...
\input{tables/T_related_work_comparison.tex}
...
Learned diagonal lenses substantially change the surfaced trade-offs: identity lenses yield 158 Pareto options, diagonal lenses (uniform down-weighting) yield 142, and learned lenses (trained via one-vs-rest logistic regression on stakeholder membership) yield 245. Cross-lens Jaccard is only 0.21, indicating that lens choice is a significant design decision---not arbitrary, but consequential for which trade-offs are exposed. This supports treating lenses as auditable governance parameters (Supplementary Table~S4).
